...
def train():
    ...
    for j in range(1):
        for parm in vggFeatures[j].parameters():
            parm.requires_grad = False;
    patchPrePath = "ME_Patches";
    ...
    for e in tbar:
        ...
        for batch in range(batches):
            ...
            grad_loss_temp = mse_loss(gradient(output),gradient(img_vi));
            grad_loss_temp2 = mse_loss(gradient(output),gradient(img_ir));
            grad_loss_temp3 = mse_loss(gradient(output),gradient(label));
            
            pixel_loss_temp = mse_loss(output, img_vi);
            pixel_loss_temp2 = mse_loss(output, img_ir);
            pixel_loss_temp3 = mse_loss(output, label);
            
            ssim_loss_temp = ssim_loss(output, img_vi, normalize=True)
            ssim_loss_temp2 = ssim_loss(output, img_ir, normalize=True)
