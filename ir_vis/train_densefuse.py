...
def train():
    ...
    for e in tbar:
        ...
        for batch in range(batches):
            ...
            sum_g_ir = torch.zeros((batch_size,1,128,128));
            sum_g_vi = torch.zeros((batch_size,1,128,128));
            
            if (args.cuda):
                sum_g_ir = sum_g_ir.cuda(args.device);
                sum_g_vi = sum_g_vi.cuda(args.device);
            
            #up sampling for caculating the activity level map
            myscale = 1;
            for j in range(5):
                up = torch.nn.UpsamplingBilinear2d(scale_factor=myscale);
                g_ir = up(vggFeatures[j](img_irdup));
                g_vi = up(vggFeatures[j](img_vidup));
                ...
                sum_g_ir = sum_g_ir.add(g_ir);
                sum_g_vi = sum_g_vi.add(g_vi);
                myscale = myscale*2;
                
            sum_g_ir = sum_g_ir.div(5);
            sum_g_vi = sum_g_vi.div(5);
            
            w_ir = sum_g_ir.greater(sum_g_vi);
            w_vi = ~w_ir;
            ...
            #save the intermediate outputs. Don't forget create a "myLabel" directory.
