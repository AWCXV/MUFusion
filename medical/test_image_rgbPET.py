...
def run_demo(model, infrared_path, visible_path, output_path_root, index, fusion_type, network_type, strategy_type, ssim_weight_str, mode):
    ...
    #vi_img = imread(visible_path,mode='L'); for grayscale
    #vi_img = imread(visible_path,mode='L');
    ...
    fuseImage = ycbcr_to_rgb(fuseImage, ir_img_cb, ir_img_cr);
...
def main():
    ...
    output_path = './outputs_medical_rgb/';
    ...
    with torch.no_grad():
        ...
        for i in range(1):
            index = i + 1
            infrared_path = test_path + 'ir_rgb/' + str(index) + '.bmp'
